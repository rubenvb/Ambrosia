 ...
 *
 * Platform/common.cpp
 * Function implementations common accross OSes.
 *
 **/
...
#if _WIN32
  const string PATH = convert_to_utf8(_wgetenv(L"PATH"));
...
const string current_working_directory()
{
  ...
  if(getcwd(stackBuffer, sizeof(stackBuffer)) != nullptr)
    ...
    return stackBuffer;
  if( errno!=ERANGE )
    throw runtime_error("Cannot determine the current path.");
  ...
  throw runtime_error("Cannot determine the current path; the path is apparently unreasonably long");
}
...
time_t last_modified(const string filename)
{
